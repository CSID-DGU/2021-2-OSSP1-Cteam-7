...
def batch_inf(bert_model, enterprise_user):
    ...
    '''
    if len(dataset_dir) == 0:
        dataset_dir = 'dataset/Cleaned/*.txt'
...
def searchengine(kw_q, database='Output/question_DB.csv',strict_mode=False):
    print('searching..')
    questions_res = []
    with open(database, 'r') as dbr:
        line = dbr.readline()
        while line:
    ...
    result_svnm = 'Output/'+ct+'.search'
    with open(result_svnm, 'a+') as rs:
        for qindex, question_iter in enumerate(questions_res):
